import { contact_icon, experiences, gmail, instagram, linkedin, more, projects, tech, user } from "@/constants.js/assets"
...
import { gmailLink, instagramLink, linkedinLink } from "@/constants.js/links"
  
function NavbarLink({text, link = ''}){
    return (
        <>
            <Link className={`text-lg header-link transition hover:text-[#1d8ce3] m-3 p-[.9px] text-center rounded-[100px]`} href={link}>
                {text}
            </Link>
        </>
    )
...
export default function HomeComponent(){
    ...
    const social = [
        ...
        { text: "Instagram", icon: instagram, link: instagramLink},
    ]
                ...
                <header className="p-10 relative z-10 my-2 h-14 flex items-center justify-center mx-w-full">
                    <div className="p-4 bg-[#0b0d0e] border-[#0e1213] border rounded-[100px] navbar_">
                        {navLinks.map((navLink, index)=>{
                            return (
                                <NavbarLink text={navLink.text} link={navLink.link}/>
                            )
                        })}
                    </div>
                    ...
                    <div className="absolute right-1 top-2 more-icon">
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent style={{marginRight: "1rem"}}  className="bg-[#0c0c0c] border border-[#141414] text-white shadow-md">
                                {navLinks.map((navlink, index)=>{
                                    return (
                                        <Link href={navlink.link}>
                                            <DropdownMenuItem className="cursor-pointer navbar_opt rounded-md">
                                                <Image src={navlink.icon} className={`mx-1 ${index == 0 ? '' : 'p-1'}`}/> {navlink.text}
                                            </DropdownMenuItem>
                                        </Link>
                                    )
                                })}
                    ...
                    <section className="w-full py-20 hero">
                        ...
                        <div className="container flex flex-col items-center justify-center gap-4 px-4 md:px-6">
                            <div className="space-y-2 text-center">
                                <h1 className={styles.heading}>Hi, I'm Parth</h1>
                                    ...
                                    {social.map((app, index)=>{
                                        return (
                                            <Link href={app.link} target="blank">
                                                <Image src={app.icon} className={`w-[30px] ${index == 1 ? '' : 'mx-3'}`}/>
                                            </Link>
                                        )
                                    })}
                                </div>
